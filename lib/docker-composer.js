...
function isRunning(path, callback) {
  const command = `${utils.changeDir(path)} docker-compose ps -q`;
...
function getContainerID(path, service, callback) {
  ...
  return utils.cmd(`${utils.changeDir(path)} docker-compose ps -q ${service}`, (err, data) => {
    if (err)
  { callback(err, data); }
    else {
...
function up(path, callback, dataNotify) {
  const pid = utils.cmd(`${utils.changeDir(path)} docker-compose  up -d`, callback);
...
function down(path, callback, dataNotify) {
  const pid = utils.cmd(`${utils.changeDir(path)} docker-compose down`, callback);
  utils.docker_logs(pid, dataNotify);
...
function exec(path, service, command, callback, paramsInput, notifyCallback) {
  ...
  if (!params.detached)
    {
    pid = utils.cmd(`${utils.changeDir(path)} docker-compose exec ${service} ${command}`, (err, data) => {
      if (err)
      {
