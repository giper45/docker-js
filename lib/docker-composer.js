...
function getComposeFile(thePath, callback) {
	//Docker yaml if alternative true
	let retPath = path.join(thePath, 'docker-compose.yml');
  let retPath2 = path.join(thePath, 'docker-compose.yaml'); 
...
function dcExec(thePath, command, callback, notifyCallback) {
	let pid = utils.cmd(
		`docker-compose -f "${getComposeFile(thePath)}" ${command}`, 
				callback);
		// Notify
...
function up(thePath, callback, dataNotify) {
 
 const pid =   dcExec(thePath, 'up -d', callback, dataNotify);
 return pid;
}
...
function exec(thePath, service, command, callback, paramsInput, notifyCallback) {
  ...
  //  pid = utils.cmd(`${utils.changeDir(thePath)} docker-compose exec ${service} ${command}`, (err, data) => {
      if (err)
      {
